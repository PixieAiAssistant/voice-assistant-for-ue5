...
class _RemoteExecutionCommandConnection(object):
    ...
    def _send_message(self, message):
        ...
        try:
            ...
            raise RuntimeError(f'Remote socket send failed: {exc}') from exc
    ...
    def _receive_message(self, expected_type):
        ...
        while True:
            try:
                ...
                raise RuntimeError(f'Remote socket error during receive: {exc}') from exc
            if not part:
                raise RuntimeError('Remote connection closed by peer during receive!')
            data += part
            if len(part) < DEFAULT_RECEIVE_BUFFER_SIZE:
                break
        if data:
